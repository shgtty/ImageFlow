...
## 2024-05-19 - Avoid layout thrashing inside requestAnimationFrame
**Learning:** Even when operations are deferred to `requestAnimationFrame`, interleaving DOM writes (like `.textContent` or `.style`) with DOM reads (like `getBoundingClientRect()`) forces the browser to synchronously recalculate layout. When this happens during high-frequency events like a `mousemove` on a seekbar, it significantly degrades frame rates (Layout Thrashing).
**Action:** Always batch DOM reads and writes separately within `requestAnimationFrame`. Fetch all necessary dimensional and positional data (`getBoundingClientRect()`, `offsetWidth`, etc.) *before* applying any changes to `.textContent` or `.style`.
